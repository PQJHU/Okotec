...
def generate_combinations(model_name=None, layer_conf=None, cells=None, dropout=None, batch_size=None, timesteps=None,
    ...
    each model: dict
        batch_size, layers, name, timesteps
    layers: list of dicts
        cells, dropout, ret_seq, stateful, type
    """
    ...
    for comb_id, comb in enumerate(combinations):
        ...
        for idx, level in enumerate(comb[0]):
            ...
            if (idx + 1) == len(comb[0]):
                return_sequence = False
            if level > 0:
                layers.append({'type': model_type,
                               'cells': level,
                               'dropout': comb[1],
                               'stateful': layer_conf[idx],
    ...
    print('==================================')
    print(tabulate([['Number of model configs generated', len(combinations)]],
                   tablefmt="jira", numalign="right", floatfmt=".3f"))
    return models


def create_models(layers=None, sample_size=None, batch_size=1, timesteps=1, features=None, loss='mse',
                  optimizer='adam', forecast_length=1, model_type='lstm'):
...
def train_models(model=None, y=None, X=None, mode='train_on_batch', batch_size=1, timesteps=1,
                 epochs=1, rearrange=False, verbose=0, validation_split=0.1, early_stopping=True, min_delta=0.006,
                 patience=2):
    """ Trains the model
    ...
    if mode == 'train_on_batch':
        ...
        if (max_batch_count * batch_size) < (X.shape[0]) and verbose > 0:
            ...
            print('Dropped the last {} of {} number of obs.'.format(X.shape[0] - max_batch_count * batch_size,
                                                                    X.shape[0]))
        ...
        raise ValueError("The mode '{}' was invalid. Must be either fit or train_on_batch".format(mode))
